...
$(document).ready(function () {
  ...
  $("code.split")
    ...
    .html(function () {
      return (
        "<span>" +
        $(this).html().split("___").join("</span>___<span>") +
        "</span>"
      );
    });
  $("code.split2dots").html(function () {
    ...
    for (let i = 0; i < split.length; i++) {
      var split2 =
        '<span class="sub">' +
        split[i].split(":").join('</span>:<span class="sub">') +
        "</span>";
  ...
  $("h1.docs-heading, h2.section-heading, h3.section-heading").append(
    function () {
      var id = $(this).closest(".docs-section, .docs-article").attr("id");
      return (
        '<a class="anchorjs-link" aria-label="Anchor" href="#' +
        id +
